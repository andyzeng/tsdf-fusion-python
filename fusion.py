...
def get_view_frustum(depth_im: np.ndarray, cam_intr: np.ndarray, cam_pose: np.ndarray) -> np.ndarray:
    """Get corners of 3D camera view frustum of depth image

    Args:
        depth_im (numpy.ndarray): The depth image, with depth in meters. Shape: (H, W).
        cam_intr (numpy.ndarray): Camera Intrinsics matrix. Shape: (3, 3).
        cam_pose (numpy.ndarray): Camera Extrinsics matrix. Shape: (4, 4).

    Returns:
        numpy.ndarray: 3D co-ords of the 5 corners of the viewing fructum (including camera center). Shape: (3, 5)
    """
    assert len(depth_im.shape) == 2
